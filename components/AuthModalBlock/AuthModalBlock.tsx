...
interface IAuthModalBlockProps {
  isOpen: boolean;
  onClose: () => void;
}

export const AuthModalBlock: React.FC<IAuthModalBlockProps> = ({
  isOpen,
  onClose,
}) => {
  const [formType, setFormType] = useState<'notSelect' | 'email'>('notSelect');
  return (
      ...
      <DialogTitle>
        ...
        {formType === 'notSelect' ? (
          ...
          <p className={styles.backTitle}>
            <ArrowBack onClick={() => setFormType('notSelect')} />
            Войти черезз почту
          </p>
        )}
      </DialogTitle>
      <DialogContent>
        {formType === 'notSelect' && (
          <>
            ...
            <div className={clsx(styles.buttons, styles.bottomButtons)}>
